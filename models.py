...
class Reminder(Base):
    ...
    chat_id:int = Column(Integer, ForeignKey("chat.id"), nullable=False, index=True)
    ...
    def __init__(self, chat_id:Integer, when:datetime, from_user:String, 
        ...
        self.is_daily = target_user==None and subject==None
    ...
    def get_time(self):
        return self.when.astimezone(tz=ZoneInfo(self.chat.time_zone)) #BUG chat isn't populated
    
    def format_string(self):
        when = self.when
        if self.is_daily: return f"{when.hour:02d}:{when.minute:02d}"
        else: 
    ...
    def __repr__(self):
        more = f"is_daily={self.is_daily}" if self.is_daily else f"target={self.target_user}, subject={self.subject}"
        return f"Reminder({self.name}, {more})"
    
    def __lt__(self, other):
        if self.is_daily: return self.when.hour * 60 + self.when.minute < other.when.hour * 60 + other.when.minute
        else: return self.when < other.when
