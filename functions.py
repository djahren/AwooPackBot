...
def parse_date(date_string: str) -> datetime:
    ...
    elif date_match_intl is not None:
...
def parse_reminder(chat_id: int, from_user: str, args: tuple[str]) -> db.Reminder:
    ...
    for index, word in enumerate([a.lower() for a in args]):
        ...
        if word in keywords:
            ...
            if indicies[cur_kw]["finished"]:
                continue

            if indicies[cur_kw]["from"] == -1:
                indicies[cur_kw]["from"] = index
    ...
    if "at" not in indicies and "in" not in indicies:
        return False

    if reminder.target_user.lower() == "me":
        reminder.target_user = reminder.from_user
    else:
        reminder.target_user = reminder.target_user.lstrip("@")
